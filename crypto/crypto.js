...
const deployCurrencys = async (data)=>{
    ...
    document.getElementById('cryptoDiv').innerHTML=fragment;
        ...
        document.getElementById('cryptoDiv').innerHTML= `
            <tr>
                <td></td>
                <h6 style="text-align:center;width:100%;">Sorry! We cannot find any currency</h6>
                <td></td>
                <td></td>
            </tr>`
        }
...
search.addEventListener('input', (e)=>{
    let newData = crypto.filter(element => element.name.includes(e.target.value) || element.symbol.includes(e.target.value));
    if(e.target.value == ''){
        deployCurrencys(crypto);
    }else{
